...
exec > >(tee -a logs/Peloton-LED.log) 2>&1
...
usage() {
    cat <<USAGE
    Usage: ./LLL-install.sh [-a | --skip-all] [-c | --skip-config] [-m | --skip-matrix]
...
if [ "$SKIP_VENV" = false ]; then
    ...
    if [ -f "./peloton_led.py" ] && ! grep -q "#\!/" peloton_led.py; then
        if [[ "$OSTYPE" == "darwin"* ]]; then
            sed -i '' '1i\'$'\n''#!'"$(which python3)"$'\n' peloton_led.py
        elif [ "$NO_SUDO" = false ]; then
            sed -i "1i #\!/usr/bin/sudo $(which python3)" peloton_led.py
        else
            sed -i "1i #\!$(which python3)" peloton_led.py
        fi
        chmod +x peloton_led.py
        git add peloton_led.py
    fi
fi
